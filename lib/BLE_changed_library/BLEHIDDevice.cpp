...
BLEHIDDevice::BLEHIDDevice(BLEServer* server) {
    ...
    m_customdataService = server->createService(BLEUUID((uint16_t) 0x3000));
    m_customwriteService = server ->createService(BLEUUID((uint16_t) 0x3001));
    ...
    m_customnotifyCharacteristic = m_customdataService->createCharacteristic((uint16_t) 0x2FFF, BLECharacteristic::PROPERTY_NOTIFY | BLECharacteristic::PROPERTY_READ);


    BLE2902 *customnotifyDescriptor = new BLE2902();

    m_customnotifyCharacteristic->setAccessPermissions(ESP_GATT_PERM_READ_ENCRYPTED | ESP_GATT_PERM_WRITE_ENCRYPTED);
    customnotifyDescriptor->setAccessPermissions(ESP_GATT_PERM_READ_ENCRYPTED | ESP_GATT_PERM_WRITE_ENCRYPTED);

    customnotifyDescriptor ->setNotifications(true);
    m_customnotifyCharacteristic -> addDescriptor(customnotifyDescriptor);

    m_customdefaultvaluesCharacteristic = m_customdataService->createCharacteristic((uint16_t) 0x2FFA,  BLECharacteristic::PROPERTY_READ);
    m_customdefaultvaluesCharacteristic->setAccessPermissions(ESP_GATT_PERM_READ_ENCRYPTED | ESP_GATT_PERM_WRITE_ENCRYPTED);

    m_customcurrentvaluesCharacteristic = m_customdataService->createCharacteristic((uint16_t) 0x2FFB,  BLECharacteristic::PROPERTY_READ);
    m_customcurrentvaluesCharacteristic->setAccessPermissions(ESP_GATT_PERM_READ_ENCRYPTED | ESP_GATT_PERM_WRITE_ENCRYPTED);

    m_customwritedataCharacteristic = m_customwriteService->createCharacteristic((uint16_t) 0x2FFC, BLECharacteristic::PROPERTY_WRITE);
	m_customwritedataCharacteristic->setAccessPermissions(ESP_GATT_PERM_READ_ENCRYPTED | ESP_GATT_PERM_WRITE_ENCRYPTED);

    m_customwriteiclickdataCharacteristic = m_customwriteService->createCharacteristic((uint16_t) 0x2FFD, BLECharacteristic::PROPERTY_WRITE);
	m_customwriteiclickdataCharacteristic->setAccessPermissions(ESP_GATT_PERM_READ_ENCRYPTED | ESP_GATT_PERM_WRITE_ENCRYPTED);
...
void BLEHIDDevice::startServices() {
    ...
    m_customdataService->start();
    m_customwriteService->start();
}
...
BLECharacteristic* BLEHIDDevice::featureReport(uint8_t reportID) {
    BLECharacteristic* featureReportCharacteristic = m_hidService->createCharacteristic((uint16_t) 0x2a4d, BLECharacteristic::PROPERTY_READ);
...
void BLEHIDDevice::setnotifyCharacteristic(){
    std::string payload = "";

    float charge = battery_chec.charging_value(); 
    bool charging_status = battery_chec.charging_status(charge);
    int battery_charge = battery_chec.battery_check();
    ...
    m_customnotifyCharacteristic->setValue(payload);
    m_customnotifyCharacteristic->notify();
};
...
BLECharacteristic* BLEHIDDevice::getwritedataCharacteristic() {
    return m_customwritedataCharacteristic;
}

BLECharacteristic* BLEHIDDevice::getdefaultvaluesCharacteristic() {
    return m_customdefaultvaluesCharacteristic;
}

BLECharacteristic* BLEHIDDevice::getwriteiclickdataCharacteristic() {
    return m_customwriteiclickdataCharacteristic;
}
